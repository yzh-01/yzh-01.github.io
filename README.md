...
- 首页顶部为可回看的水滴序章：水滴撞击雾窗后形成涟漪，花园从软边波纹中扩散显露，并加入湿墨标题、折射镜缘、雨痕与滚动退场
...
首页序章只在首页生成。它占据顶部一屏：一滴水从画面上方落向雾窗，撞击后产生多层苔绿色涟漪和轻微镜头回弹，猫与花园从软边波纹内部向外扩散显露，标题以湿墨扫光质感出现，环境薄雾随后缓慢漂移；鼠标或触控可短暂擦开雾面，并显示不完整的玻璃折射镜缘。序章离开视口后自动复位，重新向上滚动时再次播放。不支持遮罩的浏览器自动回退为整幅柔和转场；移动端减少雾层与雨痕数量并缩小涟漪和镜缘尺寸；低动态偏好下保留静态序章，不执行水滴、涟漪、雨痕和视差动画。
